...
A classe `Chip8` implementa o padrão de design **Facade** (Fachada). Ela fornece uma interface simplificada para um subsistema complexo (o emulador CHIP-8 completo). Em vez de ter que interagir diretamente com CPU, memória, display e teclado, o usuário interage apenas com a classe `Chip8`, que coordena todas as operações internamente.

Isso traz várias vantagens:
- **Simplicidade**: A interface é mais fácil de usar
- **Desacoplamento**: O código que usa o emulador não precisa conhecer os detalhes internos
- **Manutenibilidade**: Mudanças internas não afetam o código que usa a classe
- **Testabilidade**: É mais fácil testar o sistema como um todo

### Resumo

A classe `Chip8` é o ponto de entrada principal do emulador. Ela:
- Coordena todos os componentes (CPU, memória, display, teclado)
- Fornece uma interface simples e limpa para controlar o emulador
- Encapsula a complexidade interna do sistema
- Permite configurar comportamentos específicos (quirks) de diferentes implementações do CHIP-8
- Facilita a integração com interfaces de usuário e outras aplicações

É através dessa classe que o emulador ganha vida e se torna utilizável como um sistema completo e funcional.
